...
import { getCategoryIcon, WarningIcon, SearchIcon, PlusIcon, MinusIcon, ShoppingCartIcon } from '../Components/Icons';
...
function Menu() {
    ...
    return (
        <div className="space-y-6">
            {/* Cart Button - Floating Circular Button */}
            {getCartCount() > 0 && (
                <button
                    onClick={() => setShowCheckout(true)}
                    className="fixed bottom-24 sm:bottom-6 left-1/2 sm:left-auto sm:right-6 transform -translate-x-1/2 sm:translate-x-0 z-[60] w-16 h-16 sm:w-14 sm:h-14 bg-gradient-to-br from-red-600 to-red-700 text-white rounded-full shadow-2xl hover:from-red-700 hover:to-red-800 transition-all duration-200 flex items-center justify-center hover:scale-110 active:scale-95 border-2 border-red-500/50"
                    aria-label="مشاهده سبد خرید"
                >
                    <div className="relative">
                        <ShoppingCartIcon className="w-6 h-6 sm:w-5 sm:h-5" />
                        {/* Badge with count */}
                        <span className="absolute -top-2 -right-2 bg-white text-red-600 text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center border-2 border-red-600 shadow-lg">
                            {getCartCount() > 99 ? '99+' : getCartCount().toLocaleString('fa-IR')}
                        </span>
                    </div>
                </button>
            )}
            ...
            {/* Branch Selector */}
            {branches.length > 0 && (
                <div className="cafe-card rounded-xl p-4 relative z-10">
                    <Select
                        label="انتخاب شعبه"
                        value={branchId || ''}
