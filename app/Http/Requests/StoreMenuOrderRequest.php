...
class StoreMenuOrderRequest extends FormRequest
{
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            // Either reservation_id or session_id must be provided
            if (!$this->has('reservation_id') && !$this->has('session_id')) {
                $validator->errors()->add('reservation_id', 'باید یا reservation_id یا session_id را ارسال کنید.');
            }

            // Both cannot be provided at the same time
            if ($this->has('reservation_id') && $this->has('session_id')) {
                $validator->errors()->add('reservation_id', 'نمی‌توانید همزمان reservation_id و session_id را ارسال کنید.');
            }
        });
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'session_id.exists' => 'سانس انتخاب شده معتبر نیست.',
        ];
    }
}
